function loadTileset(setID: string): void {
  for (const [key, value] of Object.entries(DATA_GAME_TILESETS[setID])) {
    // Reset
    const tile = document.getElementById('tile-' + key) as HTMLElement;
    ...
    // Set Action Name
    const s = parts[0] || '';
    const info = document.getElementById('tile-info-' + key) as HTMLElement;
    info.innerHTML = s;
